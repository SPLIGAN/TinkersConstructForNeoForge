 ...
 * Interaction-heavy modifiers include types under {@link slimeknights.tconstruct.tools.modules.interaction.HarvestModule}'s package.
 */
public enum ZoomModule implements ModifierModule, GeneralInteractionModifierHook, KeybindInteractModifierHook, UsingToolModifierHook, EquipmentChangeModifierHook {
  ...
  private static void setZoom(ModifierEntry modifier, LivingEntity living, float amount) {
    TinkerDataCapability.getData(living).computeIfAbsent(TinkerDataKeys.FOV_MODIFIER).set(modifier.getId().getLocation(), amount);
  }
