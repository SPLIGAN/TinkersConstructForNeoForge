...
public class BlockTagProvider extends BlockTagsProvider {
  ...
  private void addCommon() {
    ...
    addGlass(TinkerCommons.clearStainedGlass, "glass/", tag(Tags.Blocks.GLASS_BLOCKS_CHEAP));
    addGlass(TinkerCommons.clearStainedGlassPane, "glass_panes/", tag(TinkerTags.Blocks.STAINED_GLASS_PANES));
    ...
    IntrinsicTagAppender<Block> silicaGlass = tag(TinkerTags.Blocks.GLASS_SILICA);
  ...
  private void addTools() {
    ...
    this.tag(TinkerTags.Blocks.WORKBENCHES)
        ...
        .addOptionalTag(ResourceLocation.parse("forge:workbench")); // optional compat for mods still using the Forge namespaced tag
  ...
  private void addHarvest() {
    ...
    for (DirtType dirt : DirtType.values()) {
      for (FoliageType grass : FoliageType.values()) {
        ...
        // cannot use tier sorting registry as it's not init during datagen; vanilla removed Tiers#getLevel — compare mining tiers explicitly
        Tiers tier;
        int dirtLvl = tierMiningLevel(dirtTier);
        int grassLvl = tierMiningLevel(grassTier);
        if (dirtLvl == grassLvl) {
          tier = dirtTier.ordinal() > grassTier.ordinal() ? dirtTier : grassTier;
        } else {
          tier = dirtLvl > grassLvl ? dirtTier : grassTier;
        }
  ...
  /** Vanilla mining tier ordering for comparing {@link Tiers} without {@code getLevel()} (removed in modern versions). */
  private static int tierMiningLevel(Tiers tier) {
    return switch (tier) {
      case WOOD, GOLD -> 0;
      case STONE -> 1;
      case IRON -> 2;
