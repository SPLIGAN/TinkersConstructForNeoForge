...
import static slimeknights.tconstruct.common.TinkerTags.Items.BODY;
...
public class ItemTagProvider extends ItemTagsProvider {
  ...
  private void addCommon() {
    ...
    // glass
    copy(TinkerTags.Blocks.GLASS_SILICA, TinkerTags.Items.GLASS_SILICA);
    ...
    copy(TinkerTags.Blocks.STAINED_GLASS_PANES, TinkerTags.Items.STAINED_GLASS_PANES);
  ...
  private void addWorld() {
    IntrinsicTagAppender<Item> heads = this.tag(TagKey.create(Registries.ITEM, ResourceLocation.withDefaultNamespace("skulls")));
  ...
  private void addSmeltery() {
    ...
    moltenTools(TinkerFluids.moltenGold).minecraft("golden")
      ...
      .add(4, false, ResourceLocation.withDefaultNamespace("golden_boots"))
    ...
    moltenTools(TinkerFluids.moltenDiamond).minecraft().excavatorSpikeMaul().crowbar().fdKnife()
      .add(4, false, ResourceLocation.withDefaultNamespace("diamond_boots"))
  ...
  /** Vanilla slot tags ({@link ItemTags}); NeoForge renamed/removed legacy {@code Tags.Items.ARMORS_*}. */
  private TagKey<Item> getVanillaArmorSlotTag(ArmorItem.Type slotType) {
    return switch (slotType) {
      ...
      // Vanilla 1.21.1 has no ItemTags.#BODY_*; enchanting uses this umbrella for wearable armor slots.
      case BODY -> ItemTags.ARMOR_ENCHANTABLE;
    };
  }
  ...
  private void addArmorTags(EnumObject<ArmorItem.Type,? extends Item> armor, TagKey<Item>... tags) {
    armor.forEach((type, item) -> {
      ...
      this.tag(getVanillaArmorSlotTag(type)).add(item);
    });
  }
