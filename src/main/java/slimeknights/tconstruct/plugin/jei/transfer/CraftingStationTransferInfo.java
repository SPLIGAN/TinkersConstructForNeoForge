...
public class CraftingStationTransferInfo implements IRecipeTransferInfo<CraftingStationContainerMenu, RecipeHolder<CraftingRecipe>> {
  ...
  @Override
  public List<Slot> getInventorySlots(CraftingStationContainerMenu container, RecipeHolder<CraftingRecipe> recipeHolder) {
  ...
  @Override
  public List<Slot> getRecipeSlots(CraftingStationContainerMenu container, RecipeHolder<CraftingRecipe> recipeHolder) {
    List<Slot> slots = new ArrayList<>();
    for (int i = 0; i < 9; i++) {
      slots.add(container.getSlot(i));
    }
