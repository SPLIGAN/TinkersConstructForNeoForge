...
public record FluidModifierModel(Material small, @Nullable Material large, ToolTankHelper tankHelper) implements SimpleModifierModel {
  ...
  /** Cache key for {@link #getCacheKey(IToolStackView, ModifierEntry)} — uses component hash instead of legacy fluid NBT. */
  private record CacheKey(Fluid fluid, int fluidAndComponentsHash) {}
  ...
  public Object getCacheKey(IToolStackView tool, ModifierEntry modifier) {
    ...
    if (!fluid.isEmpty()) {
      return new CacheKey(fluid.getFluid(), FluidStack.hashFluidAndComponents(fluid));
    }
    return null;
  }
