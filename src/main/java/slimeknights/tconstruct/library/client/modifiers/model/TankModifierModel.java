...
public class TankModifierModel implements ModifierModel {
  ...
  private record CacheKey(Fluid fluid, int fluidAndComponentsHash, boolean partial) {}
  ...
  public Object getCacheKey(IToolStackView tool, ModifierEntry modifier) {
    ...
    if (!fluid.isEmpty()) {
      return new CacheKey(fluid.getFluid(), FluidStack.hashFluidAndComponents(fluid), fluid.getAmount() + tolerance < helper.getCapacity(tool));
    }
    return null;
  }
