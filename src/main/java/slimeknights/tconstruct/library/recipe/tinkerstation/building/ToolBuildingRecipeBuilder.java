...
public class ToolBuildingRecipeBuilder extends AbstractRecipeBuilder<ToolBuildingRecipeBuilder> {
  ...
  public void save(RecipeOutput consumerIn, ResourceLocation id) {
    var advancementHolder = this.buildOptionalAdvancementHolder(consumerIn, id, "parts");
    if (tippedModifier != null) {
      ...
      consumerIn.accept(id, new TippedToolTransformRecipe(id, group, output, layoutSlot, extraRequirements.get(0), extraMaterials, tippedModifier), advancementHolder);
    } else {
      consumerIn.accept(id, new ToolBuildingRecipe(id, group, output, outputSize, layoutSlot, extraRequirements, partsOverride, extraMaterials), advancementHolder);
    }
  }
}
