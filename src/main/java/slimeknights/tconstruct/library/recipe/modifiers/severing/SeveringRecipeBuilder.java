...
public class SeveringRecipeBuilder extends AbstractRecipeBuilder<SeveringRecipeBuilder> {
  ...
  public void save(RecipeOutput consumer, ResourceLocation id) {
    var advancementHolder = this.buildOptionalAdvancementHolder(consumer, id, "severing");
    if (childOutput != null) {
      consumer.accept(id, new AgeableSeveringRecipe(id, ingredient, output, childOutput, baseChance, lootingBonus), advancementHolder);
    } else {
      consumer.accept(id, new SeveringRecipe(id, ingredient, output, baseChance, lootingBonus), advancementHolder);
    }
  }
}
