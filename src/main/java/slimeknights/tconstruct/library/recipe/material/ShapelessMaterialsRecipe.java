...
public class ShapelessMaterialsRecipe extends ShapelessRecipe implements MaterialsCraftingTableRecipe {
  ...
  public ItemStack assemble(CraftingInput inventory, HolderLookup.Provider provider) {
    return ShapedMaterialsRecipe.assemble(super.assemble(inventory, provider), inventory, getIngredients(), partCount, false, extraMaterials);
  }
  ...
  public static class Serializer implements LoggingRecipeSerializer<ShapelessMaterialsRecipe> {
    ...
    public ShapelessMaterialsRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
      ShapelessRecipe vanilla = RecipeSerializer.SHAPELESS_RECIPE.codec().codec().parse(JsonOps.INSTANCE, json).getOrThrow(JsonSyntaxException::new);
