...
public record MaterialTagSource(MaterialManager manager) implements TagSource<IMaterial> {
  ...
  public List<ResourceLocation> keysInTag(TagKey<IMaterial> tag) {
    ...
    return entries.stream().map(IMaterial::getIdentifier).map(MaterialId::getLocation).collect(Collectors.toList());
  }
  ...
  public Stream<ResourceLocation> valueKeys() {
    return manager.getAllMaterials().stream().map(IMaterial::getIdentifier).map(MaterialId::getLocation);
  }
}
