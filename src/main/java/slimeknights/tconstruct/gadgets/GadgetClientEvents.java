...
public class GadgetClientEvents extends ClientEventBase {
  ...
  static void registerModels(RegisterAdditional event) {
    FancyItemFrameRenderer.LOCATIONS_MODEL.values().forEach(rl -> event.register(ModelResourceLocation.standalone(rl)));
    FancyItemFrameRenderer.LOCATIONS_MODEL_MAP.values().forEach(rl -> event.register(ModelResourceLocation.standalone(rl)));
  }
